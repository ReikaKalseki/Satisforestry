	...
	while (c2 != null && path.size() < 4) {
				...
				for (int i = -r; i <= r; i++) {
					for (int k = -r; k <= r; k++) {
						if (i*i+k*k <= r*r) {
							...
							if (Satisforestry.isPinkForest(world, ddx, ddz) && DecoratorPinkForest.isTerrain(world, ddx, top, ddz) && this.countSolidSides(world, ddx, top, ddz) >= 2) {
		...
		private void carve(HashMap<Coordinate, BlockCandidate> map, World world, Block put, boolean force) {
			for (Entry<Coordinate, BlockCandidate> e : map.entrySet()) {
				...
				int dy = y+2;
				if (world.getBlock(c2.xCoord, dy, c2.zCoord) == Satisforestry.log && world.getBlockMetadata(c2.xCoord, dy, c2.zCoord) <= 3) {
					while (dy >= 0 && (ReikaWorldHelper.softBlocks(world, c2.xCoord, dy, c2.zCoord) || world.getBlock(c2.xCoord, dy, c2.zCoord) == Satisforestry.log)) {
	...
	private static class SandFinder extends BreadthFirstSearch {
		...
		private final PropagationCondition propagationCondition = new PropagationCondition() {
			...
			public boolean isValidLocation(World world, int x, int y, int z, Coordinate from) {
				return (y >= from.yCoord-1 || root.yCoord-y <= 2) && y >= highestY-2 && Satisforestry.isPinkForest(world, x, z) && world.getBlock(x, y, z) == Blocks.sand;
			}
