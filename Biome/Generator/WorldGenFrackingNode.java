	...
	private boolean tryPlace(World world, int x, int z, Random rand) {
		...
		for (double ang = 0; ang < 360; ang += 2.5) {
			...
			for (double r = 9; r <= 16; r += 0.25) {
				...
				int y = DecoratorPinkForest.getTrueTopAt(world, dx, dz);
				if (y >= minY && !FrackerStructure.isUnderFrackingFootprint(world, dx-x, dz-z)) {
					if (this.isValidAirBlock(world, dx, y+1, dz) && this.isValidGroundBlock(world, dx, y, dz) && this.isValidGroundBlock(world, dx, y-1, dz)) {
		...
		while (spawned.size() < rf.maxNodes && !take.isEmpty()) {
			...
			for (Coordinate c2 : spawned) {
				if (c2.getTaxicabDistanceTo(c) <= 5 || Math.abs(Math.toDegrees(Math.atan2(c2.zCoord-c.zCoord, c2.xCoord-c.xCoord))) <= 5) {
					close = true;
					break;
				}
			}
			...
			for (double d = 0; d <= 1; d += 0.2) {
				this.clearArea(world, MathHelper.floor_double(x+(c.xCoord-x)*d), y, MathHelper.floor_double(z+(c.zCoord-z)*d), 2, root);
			}
			...
			for (int k = -(int)r; k <= r; k++) {
				if (i*i+k*k <= r*r+0.5 && (root == null || x != root.xCoord || z != root.zCoord)) {
					for (int dy = y-2; dy <= y; dy++) {
						...
						if (b != SFBlocks.FRACKNODE.getBlockInstance() && b != SFBlocks.FRACKNODEAUX.getBlockInstance() && b != SFBlocks.CAVESHIELD.getBlockInstance())
