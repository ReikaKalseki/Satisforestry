	...
	public boolean generate(World world, Random rand, int x, int y, int z) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (isReplaceable(world, dx, dy, dz) && !Satisforestry.pinkforest.isRoad(world, dx, dz)) {
				...
				BlockKey ore = DecoratorPinkForest.generateOreClumpAt(world, dx, dy, dz, rand, OreSpawnLocation.BORDER, size, (w, c) -> isReplaceable(w, c.xCoord, c.yCoord, c.zCoord) && DecoratorPinkForest.getTrueTopAt(w, c.xCoord, c.zCoord) >= dy-4);
				if (ore != null) {
