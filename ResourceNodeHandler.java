...
import Reika.Satisforestry.Config.ResourceItem.ResourceItemView;
		...
		if (r instanceof ResourceEntry) {
			...
			Collections.sort(c, new Comparator<ResourceItemView>() {
				...
				public int compare(ResourceItemView o1, ResourceItemView o2) {
					return ReikaItemHelper.comparator.compare(o1.getItem(), o2.getItem());
				}
			});
			for (ResourceItemView ri : c) {
				api.drawItemStack(api.itemRenderer, fr, ri.getItem(), 3, 26+dy);
				String n = ri.getItem().getDisplayName();
				fr.drawString(n, 26, 28+dy, 0x000000);
				fr.drawString("Drop Weight: "+ri.weight, 26, 38+dy, 0x000000);
				String counts = ri.minCount == ri.maxCount ? ri.minCount+"x" : ri.minCount+"x - "+ri.maxCount+"x";
				if (GuiScreen.isShiftKeyDown()) {
